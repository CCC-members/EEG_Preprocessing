...
EEG.nbchan                      = length(EEG.chanlocs);
if verbosity    
    spectopo(EEG.data,0,EEG.srate,'limits',[0 max_freq NaN NaN -10 10],'chanlocs',EEG.chanlocs,'chaninfo',EEG.chaninfo,'freq',freq_list);
    FigList                     = findobj(allchild(0), 'flat', 'Type', 'figure');
    FigHandle                   = FigList(1);
...
for i=1:length(EEGs)
    ...
    if verbosity
        ...
        spectopo(EEG_cleaned.data,0,EEG_cleaned.srate,'limits',[0 max_freq NaN NaN -10 10],'chanlocs',EEG_cleaned.chanlocs,'chaninfo',EEG_cleaned.chaninfo,'freq',freq_list);
        FigList                 = findobj(allchild(0), 'flat', 'Type', 'figure');
        FigHandle               = FigList(1);
